...
const SocialIcon = ({ children, href }: { children: React.ReactNode, href: string }) => (
    <a href={href} target="_blank" rel="noopener noreferrer" className="text-slate-500 hover:text-emerald-400 transition-colors">
        {children}
    </a>
);
...
export const AppFooter = () => {
  return (
    <footer className="bg-slate-950 border-t border-slate-900 py-6 px-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        
        {/* Logo y Copyright */}
        <div className="flex items-center gap-4">
          <Logo variant="symbol" className="w-6 h-6 opacity-50" />
          <p className="text-slate-500 text-xs">
            © {new Date().getFullYear()} Sporvit.
          </p>
        </div>

        {/* Links Legales Rápidos */}
        <div className="flex gap-6 text-xs text-slate-500">
          <Link href="/legal/privacy" className="hover:text-slate-300">Privacidad</Link>
          <Link href="/legal/terms" className="hover:text-slate-300">Términos</Link>
          <Link href="/contact" className="hover:text-slate-300">Soporte</Link>
        </div>
