 ...
 */
export function buildUserPrompt(context: UserPlanningContext): string {
  const totalWeeks = context.planning.phases.base + 
                     context.planning.phases.build + 
                     context.planning.phases.peak + 
                     context.planning.phases.taper + 
                     context.planning.phases.recovery;

  return `Genera un plan completo de ${totalWeeks} semanas para este usuario:
...
- Ubicación: ${context.training.trainingLocation.join(", ")}
- Equipamiento: ${context.training.availableEquipment.join(", ")}
...
- Día de entrenamiento: ${context.targets.calories.trainingDay} kcal
  → Proteína: ${context.targets.macros.protein}g
  → Carbohidratos: ${context.targets.macros.carbs}g
  → Grasas: ${context.targets.macros.fat}g
  → Fibra: ${context.targets.macros.fiber || 30}g
  
- Día de descanso: ${context.targets.calories.restDay} kcal
  → Proteína: ${Math.round(context.targets.macros.protein * 0.95)}g
  → Carbohidratos: ${Math.round(context.targets.macros.carbs * 0.8)}g
  → Grasas: ${context.targets.macros.fat}g
  → Fibra: ${context.targets.macros.fiber || 30}g

DISTRIBUCIÓN DE FASES:
${context.planning.phases.base > 0 ? `- Base: ${context.planning.phases.base} semanas` : ""}
${context.planning.phases.build > 0 ? `- Build: ${context.planning.phases.build} semanas` : ""}
${context.planning.phases.peak > 0 ? `- Peak: ${context.planning.phases.peak} semanas` : ""}
${context.planning.phases.taper > 0 ? `- Taper: ${context.planning.phases.taper} semana(s)` : ""}
${context.planning.phases.recovery > 0 ? `- Recovery: ${context.planning.phases.recovery} semana(s)` : ""}

INICIO DEL PLAN: ${context.startPreferences.startDate}
